...
@section('content')
    ...
    <div class="max-w-3xl mx-auto">
        ...
        <div class="mb-6">
            <a href="{{ route('reader.posts.index') }}" class="text-sm text-gray-500 hover:text-indigo-600">← Back to
                Posts</a>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            {{-- Cover Image --}}
            @if ($post->cover_image)
                <img src="{{ asset('storage/' . $post->cover_image) }}" class="w-full h-56 object-cover">
            @endif
            ...
            <div class="p-6">
                ...
                <div class="flex items-center gap-2 mb-3">
                    @if ($post->isPremium())
                        ...
                        <span
                            class="bg-gray-100 text-gray-600 text-xs px-2 py-1 rounded-full">${{ number_format($post->price, 2) }}</span>
                    @else
                ...
                @else
                    ...
                    <div class="mt-6 border border-indigo-100 bg-indigo-50 rounded-xl p-6 text-center">
                        ...
                        <p class="text-sm text-gray-500 mb-6">Unlock this post for
                            <strong>${{ number_format($post->price, 2) }}</strong></p>
                        ...
                        <div class="flex items-center justify-center gap-4">
                            ...
                            {{-- SSLCommerz --}}
                            <form method="POST" action="{{ route('reader.sslcommerz.initiate', $post) }}">
                                @csrf
                                <button type="submit"
                                    class="inline-block bg-green-600 text-white text-sm px-6 py-2 rounded-lg hover:bg-green-700 transition">
                                    🏦 Pay with SSLCommerz
                                </button>
                            </form>
                        </div>
